...
class PrometheusClient:
    ...
    async def make_request(
        ...
        while retry_count < max_retries:
            try:
                # Create session with optional profile or environment variables
                if profile:
                    session = boto3.Session(profile_name=profile, region_name=region)
                elif os.environ.get('CROSS_ACCOUNT_ROLE_ARN'):
                    # Use environment variables for cross-account access
                    sts = boto3.client('sts', region_name=region)
                    assumed_role = sts.assume_role(
                        RoleArn=os.environ['CROSS_ACCOUNT_ROLE_ARN'],
                        RoleSessionName='prometheus-mcp-session',
                        ExternalId=os.environ.get('CROSS_ACCOUNT_EXTERNAL_ID')
                    )
                ...
                if profile:
                    ...
                    logger.debug(f'Using default AWS credentials')
...
def create_server():
    """Create and return the FastAPI server instance for HTTP deployment."""
    return mcp.app


def get_prometheus_client(region_name: Optional[str] = None, profile_name: Optional[str] = None):
    """Create a boto3 AMP client with optional profile or environment variables.
    ...
    try:
        ...
        if profile_name:
            ...
            logger.info('Using cross-account role from environment variables')
            sts = boto3.client('sts', region_name=region)
            assumed_role = sts.assume_role(
                RoleArn=os.environ['CROSS_ACCOUNT_ROLE_ARN'],
                RoleSessionName='prometheus-mcp-session',
        ...
        # Create AMP client
        amp_client = session.client('amp', config=config, region_name=region)
        
        return amp_client
        
    except Exception as e:
        logger.error(f'Failed to create Prometheus client: {str(e)}')
        raise
